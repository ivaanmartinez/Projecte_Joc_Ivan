...
export class Level {
  ...
  spawnLifeIcons() {
    // URL de una imagen de corazón de GitHub
    const heartIconUrl =
      "https://raw.githubusercontent.com/photonstorm/phaser-examples/master/examples/assets/particles/red.png"

    for (let i = 0; i < 3; i++) {
      // Crear un corazón rojo usando gráficos de Phaser
      const lifeIcon = this.scene.add.image(0, 0, "coin")
      lifeIcon.setOrigin(0, 0)
      lifeIcon.setTint(0xff0000) // Color rojo
      lifeIcon.setPosition(this.keyIcon.x + this.keyIcon.width + 10 + i * (lifeIcon.width + 5), 50)
      this.lifeIcons.push(lifeIcon)
      this.groups.hud.add(lifeIcon)
    }
  }
  ...
  spawnScoreText() {
    ...
    this.scene.cache.bitmapFont.add("font:numbers", Phaser.GameObjects.RetroFont.Parse(this.scene, config))
  }
  ...
  spawnPlatform(platform) {
    const _platform = this.scene.add.sprite(platform.x, platform.y, platform.image)
  ...
  spawnEnemyWalls(platforms) {
    platforms.forEach((platform) => {
      ...
      const right = this.spawnEnemyWall(platform.x + platform.width, platform.y, "right")
    })
  }
